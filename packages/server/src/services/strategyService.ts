...
export class StrategyService {
    ...
    public getAllStrategyMetaData(): StrategyMetaData[] {
        const dirContents = fs.readdirSync(this.strategyDir);
        const dirFiles = dirContents.filter((fileOrDir: string) => {
            const fullPath = node_path.join(this.strategyDir, fileOrDir);
            const stat = fs.statSync(fullPath);
            return stat.isFile();
        });
        return dirFiles.map<StrategyMetaData>((file) => {
            return this.getMeta(file);
        });
    }
}
